...
mod xwing {
        ...
        > for XWing
    {
        fn keygen(
            ...
            #[cfg(hax)]
            {
                let key_pair = libcrux_ml_kem::mlkem768::generate_key_pair(*rand_m);
                ek_m.copy_from_slice(key_pair.pk());
                dk_m.copy_from_slice(key_pair.sk());
            }
            #[cfg(not(hax))]
        ...
        fn encaps(
            ...
            #[cfg(hax)]
            {
                let pk_m: libcrux_ml_kem::mlkem768::MlKem768PublicKey = ek_m.into();
                let (ct_result, ss_result) = libcrux_ml_kem::mlkem768::encapsulate(&pk_m, *rand_m);
                ct_m.copy_from_slice(ct_result.as_ref());
                ss_m.copy_from_slice(ss_result.as_ref());
            }
            #[cfg(not(hax))]
        ...
        fn decaps(
            ...
            #[cfg(hax)]
            {
                let sk_m: libcrux_ml_kem::mlkem768::MlKem768PrivateKey = dk_m.into();
                let ct_result: libcrux_ml_kem::mlkem768::MlKem768Ciphertext = ct_m.into();
                let ss_result = libcrux_ml_kem::mlkem768::decapsulate(&sk_m, &ct_result);
                ss_m.copy_from_slice(ss_result.as_ref());
            }
            #[cfg(not(hax))]
